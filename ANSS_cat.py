...
#==============================================================================
eq_list = ANSStools.catfromANSS(lon=[-125.4, -113.1], lat=[31.5, 43.0], minMag=5.5, dates0=[dtm.datetime(1980,01,01, tzinfo=tzutc), None], fout='../cats/mycat_Mar-05-2015.cat')

polyvectors = poly.getPolyVecs()

goodeqs = []
for eq in eq_list:
    if poly.isinPoly((eq[2]*10, eq[1]*10), polyvecs = polyvectors) == True:
        goodeqs.append(eq)

eq_ar = np.array(goodeqs)
eq_rec = np.core.records.fromarrays(eq_ar.transpose(), names=cat_names)
eq_rec.dump('../cats/cat_rec_Mar-05-2015.p')